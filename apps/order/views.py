...
# 前端传递的参数: l地址的id（addr_id） 支付方法(pay_method), 用户要购买的商品id字符串(sku_ids)
# 悲观锁：执行的时候加琐 用户抢锁
class OrderCommitView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            # 用户的订单中有几个商品，需要向ss_order_goods表中加入几条记录
            conn = get_redis_connection('default')
            cart_key = 'cart_%d' % user.id
